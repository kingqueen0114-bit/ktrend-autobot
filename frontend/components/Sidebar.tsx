    ...
    <aside className="space-y-8">
      ...
      {artistTags && artistTags.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <div className="w-full h-0.5 bg-[#292929] mb-4" />
          <div className="flex flex-wrap gap-2">
            {(artistTags as string[]).map((tag: string) => (
              <Link
                key={tag}
                href={`/artist/${encodeURIComponent(tag)}`}
                className="inline-flex items-center gap-1.5 border border-[#292929] text-[#292929] text-sm font-medium px-3 py-1.5 rounded-full hover:bg-[#292929] hover:text-white transition-colors"
              >
