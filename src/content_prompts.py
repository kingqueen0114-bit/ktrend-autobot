...
BASIC_RULES = """
基本原則:
1. 一次情報絶対主義: 公式情報を最優先する。メディアとしての信頼性を保つため事実確認（ファクトチェック）を徹底し、引用元がある場合は必ず明記すること（例：「〇〇によると〜」などの表記を含める）。
2. 文字数の充実: 他サイトと差別化するため、1000〜1500文字程度の読み応えがある内容に膨らませること。単なる事実の羅列ではなく、背景や魅力、日本での反応予測などを織り交ぜて深みを持たせる。
3. トレンドの差別化（韓国情報）: 韓国現地で今流行っているもの、これから日本でも流行りそうなもの、日本人が知ると面白いと感じるニッチで魅力的な情報を強調して書くこと。
4. 文体: 「〜です・ます」調の日本人プロライター品質。30代女性読者に響く、落ち着きがありつつも感性の高い表現を心がけること。
5. 禁止事項:
...
def build_article_prompt(title: str, snippet: str, category: str, link: str = "") -> str:
    """
